...
import timeTableResourceModel from "../models/timeTableModel.js";
...
export const getTimeTableResources = async (req, res) => {
  try {
    ...
    const resources = await timeTableResourceModel
...
export const uploadTimeTableResource = async (req, res) => {
  try {
    const { title, semester, division, resourceLink, description } = req.body;
    let url = "";
    if (!title || !semester || !division) {
      return res.status(422).json({
        status: "fail",
        error: "title, semester and division fields are required!",
      });
    }
    ...
    // Store in Database
    const newResource = new timeTableResourceModel({
      title,
      semester,
      division,
      userId: req.user._id,
...
export const updateTimeTableResource = async (req, res) => {
  try {
    ...
    const resource = await timeTableResourceModel.findById(id);
    if (!resource) {
    ...
    resource.division = division || resource.division;
...
export const deleteTimeTableResource = async (req, res) => {
  try {
    ...
    await timeTableResourceModel.findByIdAndDelete(id);
    res.status(200).json({
      status: "success",
...
export const getSingleTimeTableResource = async (req, res) => {
  try {
    const { id } = req.params;
    if (!id) {
      return res.status(404).json({

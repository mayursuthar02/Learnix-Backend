...
export const getResources = async (req, res) => {
  try {
    ...
    const resources = await resourceModel.find({userId: req.user._id}).sort({createdAt: -1});
...
export const uploadResource = async (req, res) => {
  try {
    const { title, semester, subject, resourceLink, note } = req.body;
    let url = "";
    if (!title || !semester || !subject) {
      return res.status(422).json({
        status: "fail",
        error: "title, semester and subject fields are required!",
      });
    }

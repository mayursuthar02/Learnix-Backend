...
export const getExamDetailsResources = async (req, res) => {
  try {
    ...
    const resources = await examDetailsResourceModel
...
export const uploadExamDetailsResource = async (req, res) => {
  try {
    ...
    // Store in Database
    const newResource = new examDetailsResourceModel({
      title,
      semester,
      examType,
      userId: req.user._id,
...
export const updateExamDetailsResource = async (req, res) => {
  try {
    ...
    const resource = await examDetailsResourceModel.findById(id);
    if (!resource) {
...
export const deleteExamDetailsResource = async (req, res) => {
  try {
    ...
    await examDetailsResourceModel.findByIdAndDelete(id);
    res.status(200).json({
      status: "success",
...
export const getSingleExamDetailsResource = async (req, res) => {
  try {
    const { id } = req.params;
    if (!id) {
      return res.status(404).json({
